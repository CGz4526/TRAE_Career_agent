<template>
  <div ref="containerRef" class="dark-veil-container"></div>
</template>
...
function init() {
  ...
  try {
    ...
    console.error('[DarkVeilBg] renderer init failed', e)
    return
  }
...
function cleanup() {
  ...
  if (renderer) {
    (renderer as any).gl?.canvas?.remove?.()
    renderer = null
  }
...
.dark-veil-container :deep(canvas) {
  width: 100%;
  height: 100%;
  display: block;
}
