<template>
  <div ref="ctnDom" class="aurora-container" :class="{ fixed: fixed }" :style="containerStyle" />
</template>
...
const props = withDefaults(defineProps<AuroraProps>(), {
  colorStops: () => ['#1e1b4b', '#6366f1', '#ca8a04'],
  amplitude: 1.0,
  blend: 0.5,
  speed: 1.0,
  opacity: 0.6,
...
onMounted(() => {
  ...
  resizeHandler = () => {
    ...
    const width = props.fixed ? window.innerWidth : ctn.offsetWidth
    const height = props.fixed ? window.innerHeight : ctn.offsetHeight
  ...
  program = new Program(gl, {
    ...
    uniforms: {
      ...
      uResolution: { value: [props.fixed ? window.innerWidth : ctn.offsetWidth, props.fixed ? window.innerHeight : ctn.offsetHeight] },
  ...
  const update = (t: number) => {
    ...
    if (program && renderer) {
      ...
      program.uniforms.uColorStops.value = (props.colorStops ?? ['#1e1b4b', '#6366f1', '#ca8a04']).map(
        (hex: string) => {
          const c = new Color(hex)
          return [c.r, c.g, c.b]
        },
